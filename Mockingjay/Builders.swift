...
public func failure(_ error:NSError) -> (request:URLRequest) -> Response {
  return { _ in return .failure(error) }
}

public func http(_ status:Int = 200, headers:[String:String]? = nil, download:Download=nil) -> (request:URLRequest) -> Response {
...
public func json(_ body:AnyObject, status:Int = 200, headers:[String:String]? = nil) -> (request:URLRequest) -> Response {
  return { (request:URLRequest) in
    do {
...
public func jsonData(_ data: Data, status: Int = 200, headers: [String:String]? = nil) -> (request:URLRequest) -> Response {
  return { (request:URLRequest) in
